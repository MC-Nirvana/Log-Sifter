...
## 壹、簡介
Log Sifter 是一個為 Minecraft NeoForge 伺服器與客戶端設計的模組（内部開發代號：Plana-01），專門用於過濾和遮蔽遊戲運行過程中產生的冗餘日誌資訊。該模組基於 NeoForge 平台開發，通過自定義配置規則，幫助用戶減少日誌檔案中的噪音，使重要資訊更加清晰可見。
...
## 肆、配置文件
配置檔案位於 `config` 目錄下，名稱為 `log_sifter.toml`。該檔案使用 TOML 格式進行配置，以下是一個範例配置檔案：
```toml
# Log Sifter 預設配置檔案（Log Sifter Default Configuration）

# 遮蔽規則 - 完全阻止匹配的日誌輸出（Suppress Rules - Completely prevent matching log output）
# 每一條[[suppress.rule]]表示一條遮蔽規則（Each [[suppress.rule]] represents one suppress rule）
# 格式:（Format:）
# logger: 日誌來源（logger: Log source）
# message: 訊息匹配，可以是文字或正規表示式（message: Message, can be text or regex）
# is_regex: 是否為正規表示式，預設為false（is_regex: Whether it is a regular expression, default is false）
[suppress]
...
## 捌、許可證
本項目採用 [GPL-3.0 license](LICENSE) 開源許可證
